...
- **2026-02-28**: 「投資プルダウンの表示修正およびk削除」の完了を報告しながら、実際にはファイルが更新されていない状態（または修正が不十分な状態）で放置してしまった。ユーザーから「修正されていない」との指摘を受けるまで気づけなかった。
- **事実**: `replace_file_content` の不一致エラーを適切に処理せず、部分的な修正やコマンド実行の成功のみをもって「完了」と誤認した。また、修正後のファイル内容の目視確認（`view_file`）を怠ったまま `notify_user` を送った。
- **原因**: 複数のツール実行結果が混在する中で、一部の失敗（書き込み失敗）を見落とした。また、自身の「修正したはず」という記憶に頼り、実ファイルの状態を確認するプロセスを省略した。
- **対策**: 今後、UIに関わる修正を行った後は、**必ず `view_file` でターゲット箇所のソースコードが意図通りに書き換わっているかを確認してから** `notify_user` を送ること。
...
### 【最重大】簡略/詳細モードの逆転認識（v66〜v73で発生、8バージョンに渡る連鎖障害）

- **影響範囲**: v66〜v72の7バージョンにわたり、ユーザーの意図と逆のモードに修正を適用し続けた。
- **根本原因**: v66で「色を戻して」という指示を受けた際に、ボタンテキストが「切り替え先」を示すことを理解せず、compact表示（ユーザーの言う「詳細」）の色を消してしまった。以降、コード上の `isCompact` 変数名に引っ張られ、展開表示を「詳細」、compact表示を「簡略」と逆に認識し続けた。

#### 正しいモード対応表（絶対に間違えないこと）
...
| Ver | 展開h4（簡略） | compact-machine（詳細） | 操作内容 | 正否 |
| ----- | ----------------- | ----------------------- | -------------- | ----- |
| v64 | `--primary`(紫) | なし（白） | 変更なし | ✅ |
| v65 | `--primary`(紫) | `--accent` 追加 | レイアウト調整 | ✅ |
| **v66** | `--primary`(紫) | **`--accent` 削除** | 「色戻して」を誤解 | ❌ 発端 |
| v68 | **`--text-main`(白)** | 白 | 「色を元に戻す」を更に誤解 | ❌ 悪化 |
| v72 | 白 | **`--primary`(紫)** | 逆転したまま修正 | ❌ 完全逆 |
| **v73** | **`--primary`(紫)** | **`--text-main`(白)** | 齟齬を解消 | ✅ 修正 |

#### 再発防止策（必須チェック項目）

1. **モード確認**: 修正前に必ず上記の対応表を参照し、ユーザーの言う「簡略」「詳細」がどちらのコードパスに対応するか確認する。
2. **ボタンテキストの解釈**: ボタンのテキストは常に「切り替え先」。ユーザーが言うモード名はボタンテキストの**逆**。
3. **git diffの活用**: 「戻す」「元に戻す」の指示があった場合、必ずgit diffで変更前の状態を確認してから作業する。
4. **変数名に惑わされない**: `isCompact=true` のcompact表示がユーザーの言う「詳細」である。変数名の語感で判断しない。
