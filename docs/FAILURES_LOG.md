...
- **2026-02-27 (v73)**:
  - **変更内容**: 展開表示（isCompact=false）の機種名を紫に、compact表示（isCompact=true）の機種名を白に修正。
  - **失敗分析**: UIボタンのテキストが「現在のモード名」ではなく「切り替え先のモード名」を示していることを誤解し、「簡略」と「詳細」の意味を完全に逆に捉えていた。
    - ボタン「詳細」表示 = 今は**簡略モード**（展開表示、主要項目のみ）
    - ボタン「簡略」表示 = 今は**詳細モード**（compact表示、全データ表示）
  - **教訓**: UIトグルボタンのテキストは一般的に「切り替え先」を示す。ユーザーの言う「簡略」「詳細」は常にボタンテキストの逆である。この認識を今後絶対に間違えないこと。
