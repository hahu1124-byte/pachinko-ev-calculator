...
- **v54**: モジュール分割・コードブラッシュアップ等。
- **v53**: 統計データのリアルタイム更新を実装（チェックボックス操作で即座に再計算）。
- **v52**: 履歴表示消失の最終修正（変数定義漏れによるエラー解消）、ループ構造の最適化。
- **v51**: 履歴表示フィルタの復元（v50での表示消失バグ修正）、メンテナンスルールの明文化。
- **v50**: 統計データの動的集計（選択中のみ集計）機能を実装、機種内訳の件数表記を「(○台)」に変更。
