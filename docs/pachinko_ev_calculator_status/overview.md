...
- `index.html`: UI構造。
- `css/base.css`: [NEW] ルート変数、リセット、基本レイアウト。
- `css/components.css`: [NEW] 共通部品（入力、ボタン等）。
- `css/results.css`: [NEW] 計算結果表示エリア。
- `css/history.css`: [NEW] 履歴表示・統計セクション。
- `css/animations.css`: [NEW] Keyframes、演出アニメーション。
...
- **v81 (2026-03-01)**: CSSの安全な分割。レイアウトを一切変えずに、メンテナンス性のためのモジュール化を実施。
