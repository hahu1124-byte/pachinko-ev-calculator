...
- **v46**: チェック保持ロジックをDOM優位に修正（解除操作の正常反映、不要な旧IDの自動除去）。
- **v45**: チェック保持強化（sessionStorage導入）、簡略表示ずれ修正（white-spaceのリセット）。
- **v44**: 統計パネルへの日時反映、簡略表示の先頭空白除去、チェック状態のメモリ保持。
- **v43**: 不具合修正（日時表示ON時の履歴消失バグ）、日時ボタンのデフォルトON化、LINE共有の日時同期を再点検。
- **v42**: 日時表示の全面修正（改行追加、簡略/統計モード対応）、LINE共有の日時同期、期待値演出の更なる高度化（7段階オーラ・ドクロ・拍動）。
