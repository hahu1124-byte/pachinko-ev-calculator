...
- **【NEW】開発・本番分離フロー (2026-03-01)**: `pachinko-ev-calculator` フォルダ以下の全操作は以下の2段階で進めること。
  - **Phase 1 (Staging)**: ローカルでの自動承認・即実行。修正後、必ずユーザーへ実機での動作確認を依頼せよ。
  - **Phase 2 (Production)**: ユーザー確認が「問題なし」となった場合のみ、GitHubへの最終プッシュ（本番デプロイ）を許可する。
