...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function calculateEV() {
        ...
        if (hasYutime) {
            ...
            // === I18相当: 通常持玉単価 (期待度G23の1乗と交換率係数単回除算を反映) ===
            // ユーザー指定式: ((((1R出玉 / 遊トータル確率) - (ballsPer1k / 実測回転率)) * 4) / (ballsPer1k / 250)) * G23 / 交換率
            const rawI18 = (((((rb / yutimeTotalProb) - (ballsPer1k / turnRatePer1k)) * 4) / (ballsPer1k / 250)) * yutimeExpectancy) / exchangeFactorVal;
            const i18Result = rawI18 >= 0
                ? (rawI18 / conversionFactor)
                : (rawI18 * conversionFactor);

            // === I19相当: 現金単価 (期待度G23の1乗と交換率係数単回除算を反映) ===
            // ユーザー指定式: (((1R出玉 / 遊トータル確率 * 換金単価) - (1000 / 実測回転率)) * (250 / ballsPer1k)) * G23 / 交換率
            const rawI19 = ((((rb / yutimeTotalProb * valuePerBallCashout) - (1000 / turnRatePer1k)) * (250 / ballsPer1k)) * yutimeExpectancy) / exchangeFactorVal;
