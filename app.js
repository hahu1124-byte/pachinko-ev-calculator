...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function calculateEV() {
        ...
        if (hasYutime) {
            ...
            // === J20相当: 遊タイム持玉比率単価 ===
            // ユーザー指定式: ((K18 - (1 + G23)) * 持玉比率) + ((K19 / 2 * G23 ) * (1 - 持玉比率))
            yutimeValuePerSpin = ((yutimeBallUnitPriceResult - (1 + yutimeExpectancy)) * ballRatio) + ((yutimeCashUnitPriceResult / 2 * yutimeExpectancy) * (1 - ballRatio));
